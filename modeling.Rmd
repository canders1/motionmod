...
ggplot(data=goAtrue,mapping=aes(x=world,size=cost,y=probability)) + geom_point()+facet_wrap(~p,labeller=labeller(p = facet_labs))+ ggtitle("You are going to Northampton") + xlab("World") + ylab("Joint probability of perspective and world") + labs(size="Perspective cost")
ggsave("goAtrueC.png")
```
```{r}
goEtrue <- subset(goE,probability >0)
ggplot(data=goEtrue,mapping=aes(x=world,size=cost,y=probability)) + geom_point() +facet_wrap(~p,labeller=labeller(p = facet_labs)) + ggtitle("Thera is going to Northampton") + xlab("World") + ylab("Joint probability of perspective and world") + labs(size="Perspective cost")
ggsave("goEtrueC.png")
```
```{r}
goStrue <- subset(goS,probability >0)
ggplot(data=goStrue,mapping=aes(x=world,size=cost,y=probability)) + geom_point() +facet_wrap(~p,labeller=labeller(p = facet_labs)) + ggtitle("I am going to Northampton") + xlab("World") + ylab("Joint probability of perspective and world") + labs(size="Perspective cost")
ggsave("goStrueC.png")
```
```{r}
comeEtrue <- subset(comeE,probability >0 )
ggplot(data=comeEtrue,mapping=aes(x=world,size=cost,y=probability)) + geom_point()  +facet_wrap(~p,labeller=labeller(p = facet_labs))+ ggtitle("Thera is coming to Northampton") + xlab("World") + ylab("Joint probability of perspective and world") + labs(size="Perspective cost")
ggsave('comeEtrueC.png')
```
```{r}
comeStrue <- subset(comeS,probability >0 )
ggplot(data=comeStrue,mapping=aes(x=world,size=cost,y=probability)) + geom_point()+facet_wrap(~p,labeller=labeller(p = facet_labs))+ ggtitle("I am coming to Northampton") + xlab("World") + ylab("Joint probability of perspective and world") + labs(size="Perspective cost")
ggsave('comeStrueC.png')
```
```{r}
comeA <- subset(comeA,perspective=='Sarah')
ggplot(data=comeA,mapping=aes(x=world,size=cost,y=probability)) + geom_point()+facet_wrap(~p,labeller=labeller(p = facet_labs))+ ggtitle("You are coming to Northampton") + xlab("World") + ylab("Joint probability of perspective and world") + labs(size="Perspective cost")  + guides(size=FALSE)
ggsave('comeACP.png')
```

```{r}
comeStrueP <- subset(comeS,perspective=='Jane' )
ggplot(data=comeStrueP,mapping=aes(x=world,size=cost,y=probability)) + geom_point()+facet_wrap(~p,labeller=labeller(p = facet_labs))+ ggtitle("I am coming to Northampton") + xlab("World") + ylab("Joint probability of perspective and world") + labs(size="Perspective cost")  + guides(size=FALSE)
...
ggplot(data=moveEtrue05,mapping=aes(x=world,shape=utterance,size=cost,y=probability)) + geom_point() +facet_wrap(~p,labeller=labeller(p = facet_labs)) + ggtitle("Thera is going/coming to Northampton") + xlab("World") + ylab("Joint probability of perspective and world") + labs(shape="Utterance") + guides(size=FALSE)
ggsave("moveEtrue05.png")
```
```{r}
moveEtrueC0 <- subset(moveEtrue,cost==0)
ggplot(data=moveEtrueC0,mapping=aes(x=world,size=cost,shape=utterance,y=probability)) + geom_point() +facet_wrap(~p,labeller=labeller(p = facet_labs)) + ggtitle("Thera is going/coming to Northampton") + xlab("World") + ylab("Joint probability of perspective and world") + labs(shape="Utterance",size="Cost") + guides(size=FALSE)
